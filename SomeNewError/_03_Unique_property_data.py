import pandas as pd
from _60_ADD import  \
    fn_60_ADD_PRICE_BUCKET,fn_61_ADD_LATEST_STATUS, fn_62_ADD_LATEST_PRICE, \
    fn_63_ADD_LATEST_STATUS_DATE, fn_64_ADD_FIRST_DATE, fn_65_ADD_Total_Days, fn_66_SAME_DATE_SOLD
...
class CLASS03_AGGREGATE:
    ...
    def __fn2_addtional_columns(self):
        ...
        seriesSameDaySold = ser_prop_num.apply(fn_66_SAME_DATE_SOLD, args=(raw_df_groupBy_obj,))
        seriesSameDaySold.name = 'sameDaySold'


        #------------------------------------------------------

        what2Concat = [agg_df_01, seriesLatestStatus,  seriesPrice, seriesStatusDate, seriesFirstDate, seriesNoOfDaysInMarket,seriesSameDaySold]
        df = pd.concat(what2Concat, axis=1)
        df.sort_values(by='Property_Unique_Number', ascending=True, inplace=True)

        df['priceBucket'] = fn_60_ADD_PRICE_BUCKET(df['last_price'])
